# Fueling Field Notes — 2026 season

Running log of what's tested and decided. Targets come from the training plan
(250–300 cal/hr) and sweat-data/model.json (sodium/fluid by conditions).

## Verdicts
- **Huma (original & Plus)** — PRIMARY gel. Long-proven gut tolerance, mild real-food taste.
- **Precision PF30** — SECONDARY. Neutral flavor, 120 cal, good palate reset. Zero sodium (fine — sodium is decoupled).
- **e-Gel** — REJECTED as primary (tested 7/25/26, 4 in one run): fuel profile fine but
...
- 7.0 mi / 1:42:35 at ~9,000 ft, HR 144 (disciplined B-effort). Sodium: LMNT breakfast pre-load,
  6-9 pills on course (memory fuzzy), 4 after — ledger healthy in all scenarios. 1 Huma at :45
  (adequate for a <2h effort on a full breakfast). Breakfast "sat heavy" -> reinforces
  liquid-leaning race-morning script. Station spacing forced under-drinking late; carried rig
  is the answer, not course water.
- **NEW PROTOCOL: pre-count pills into a marked container per bag/vest — count written in
